...
const countryDialingData: Array<Omit<CountryPhoneOption, "flag">> = [
...
export const COUNTRY_PHONE_OPTIONS: CountryPhoneOption[] = countryDialingData.map(
  (country) => ({
    ...country,
...
const DEFAULT_COUNTRY =
  COUNTRY_PHONE_OPTIONS.find((country) => country.code === "PK") ||
...
export const splitPhoneNumber = (value?: string | null) => {
  ...
  const matchingCountry = [...COUNTRY_PHONE_OPTIONS]
    .sort(
      (a, b) =>
        b.dialCode.replace(/-/g, "").length - a.dialCode.replace(/-/g, "").length,
    )
    .find(({ dialCode }) => safeValue.startsWith(dialCode.replace(/-/g, "")));
  ...
  const dialCodeDigits = matchingCountry.dialCode.replace(/-/g, "");
  const localNumber = stripToNumeric(safeValue.slice(dialCodeDigits.length));
...
export const combinePhoneNumber = (
  ...
  return `${country.dialCode.replace(/-/g, "")}${normalizedNumber}`;
};
